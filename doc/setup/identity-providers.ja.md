...
IdP が確認済み email を返す場合、管理者は `lore-authctl user invite --idp <provider-id> --email <address>` でユーザーを事前登録できます。

登録したユーザーの初回 login で IdP から返された確認済み email が一致すると、その login が完了します。
