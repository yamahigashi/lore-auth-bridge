...
use lore_auth_core::{
    ...
    model::{
        ...
        LoginStateInput, LoginTrustPolicy, Permission, Resource, ResourceFilter, ResourceID,
...
#[tokio::test]
async fn sql_authorization_allows_group_writer_and_denies_admin() {
...
#[tokio::test]
async fn sql_authorization_ignores_deleted_repositories() {
    let fixture = migrated_store().await;
    let store = &fixture.store;
    let user = store
        .add_user(AddUserInput {
    ...
    assert!(matches!(
        store.can_access(&user.id, &resource_id, "write").await,
...
#[tokio::test]
async fn sql_authorization_returns_not_found_for_unknown_repository() {
    let fixture = migrated_store().await;
    let store = &fixture.store;
    let user = store
        .add_user(AddUserInput {
    ...
    assert!(matches!(
        store.can_access(&user.id, &resource_id, "read").await,
        Err(CoreError::NotFound)
    ));
}

#[tokio::test]
async fn sql_authorization_applies_roles_prefix_filter_and_sorting() {
    let fixture = migrated_store().await;
    let store = &fixture.store;
    let user = store
        .add_user(AddUserInput {
